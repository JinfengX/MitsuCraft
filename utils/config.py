...
def argument_parser(epilog=None):
    ...
    Run with default config:
        $ {sys.argv[0]} --config-file configs/default/default.py --file test.ply
    Run with multi files:
        $ {sys.argv[0]} --config-file /path/to/config --file test1.ply --file test2.ply
    Search files with regular expression:
        $ {sys.argv[0]} --config-file /path/to/config --regex .*.ply
    Search files with regular expression and specified files simultaneously:
        $ {sys.argv[0]} --config-file /path/to/config --regex .*.ply --file test.ply
    Change some options in config file:
        $ {sys.argv[0]} --config-file /path/to/config --file test.ply --options work_dir=/path/to/work_dir recursive=true
    """,
...
def config_parser(file_path, args=None):
    check_file_exist(file_path)
    cfg = Config.fromfile(file_path)
    if args.options is not None:
        cfg.merge_from_dict(args.options)
    cfg.file = tuple(args.file) if args.file is not None else None
    cfg.regex = tuple(args.regex) if args.regex is not None else None
    mkdir_or_exist(cfg.save_path)
    mkdir_or_exist(osp.join(cfg.save_path, "convert"))
    mkdir_or_exist(osp.join(cfg.save_path, "image"))
    cfg.dump(osp.join(cfg.save_path, "config.py"))
    return cfg
